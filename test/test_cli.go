...
func (s *CLISuite) TestRoute(t *c.C) {
	...
	t.Assert(r.Certificate.ChainPEM(), c.Equals, strings.Trim(cert.Cert, "\n"))
	t.Assert(r.Certificate.KeyPEM(), c.Equals, strings.Trim(cert.PrivateKey, "\n"))
...
func (s *CLISuite) TestCluster(t *c.C) {
	...
	// check GitURL is configured correctly
	cluster := s.clusterConf(t)
	t.Assert(flynn("cluster", "add", "--tls-pin", cluster.TLSPin, "baz", cluster.ControllerURL, cluster.Key), Succeeds)
	cfg, err = config.ReadFile(file.Name())
	t.Assert(err, c.IsNil)
