...
func NewRepo(db *postgres.DB, artifacts *artifacts.Repo, que *que.Client) *Repo {
	return &Repo{
		db:        db,
		artifacts: artifacts,
		que:       que,
...
func releaseList(rows *pgx.Rows) ([]*ct.Release, error) {
	...
	for rows.Next() {
		release, err := common.ScanRelease(rows)
		if err != nil {
			rows.Close()
			return nil, err
		}
...
func (r *Repo) AppList(appID string) ([]*ct.Release, error) {
...
func (r *Repo) Delete(app *ct.App, release *ct.Release) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}
	...
	formations, err := common.ScanFormations(rows)
	if err != nil {
		tx.Rollback()
		return err
	}
