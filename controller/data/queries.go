...
var preparedStatements = map[string]string{
	...
	"tcp_route_list_for_update":                 tcpRouteListForUpdateQuery,
	"tcp_route_list_by_parent_ref":              tcpRouteListByParentRefQuery,
	"tcp_route_list_for_update_by_parent_ref":   tcpRouteListForUpdateByParentRefQuery,
...
UPDATE volumes SET updated_at = now(), decommissioned_at = now() WHERE app_id = $1 AND volume_id = $2 RETURNING updated_at, decommissioned_at`
	httpRouteListQuery = `
SELECT r.id, r.parent_ref, r.service, r.port, r.leader, r.drain_backends, r.domain, r.sticky, r.path, r.disable_keep_alives, r.created_at, r.updated_at, c.id, ARRAY(SELECT http_route_id FROM route_certificates WHERE certificate_id = c.id), c.chain, k.key, c.strict, c.created_at, c.updated_at FROM http_routes as r
...
WHERE deleted_at IS NULL`
	tcpRouteListForUpdateQuery   = tcpRouteListQuery + " FOR UPDATE OF tcp_routes"
	tcpRouteListByParentRefQuery = `
SELECT id, parent_ref, service, port, leader, drain_backends, created_at, updated_at FROM tcp_routes
WHERE parent_ref = $1 AND deleted_at IS NULL`
	tcpRouteListForUpdateByParentRefQuery = tcpRouteListByParentRefQuery + " FOR UPDATE OF tcp_routes"
	tcpRouteInsertQuery                   = `
...
INSERT INTO certificates (id, chain, key_id, strict)
VALUES ($1, $2, $3, $4)
ON CONFLICT (id) WHERE deleted_at IS NULL DO UPDATE SET chain = $2
RETURNING created_at, updated_at`
