...
UPDATE releases SET deleted_at = now() WHERE release_id = $1 AND deleted_at IS NULL`
	artifactListQuery = `
SELECT artifact_id, type, uri, meta, manifest, hashes, layer_url_template, created_at FROM artifacts
...
SELECT artifact_id, meta, manifest, hashes, layer_url_template, created_at FROM artifacts WHERE type = $1 AND uri = $2 AND deleted_at IS NULL`
	artifactInsertQuery = `
INSERT INTO artifacts (artifact_id, type, uri, meta, manifest, hashes, layer_url_template) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING created_at`
	artifactDeleteQuery = `
