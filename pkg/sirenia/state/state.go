...
func (x *Config) IsTunablesUpdate(y *Config) bool {
	if x == nil || y == nil {
		return false
	}
	if !peersEqual(x.Upstream, y.Upstream) {
	...
	if !peersEqual(x.Downstream, y.Downstream) {
		return false
	}
	return x.Tunables.Version != y.Tunables.Version
}
...
type Database interface {
	...
	// ValdiateTunables checks that a new set of Tunables is valid for this database
	ValidateTunables(Tunables) error
}
...
func (p *Peer) startTakeover(reason string, minWAL xlog.Position) bool {
	...
	p.startTakeoverWithPeer(reason, minWAL, &State{
		...
		Async:    newAsync,
		Deposed:  newDeposed,
		Tunables: p.Info().State.Tunables,
	})
	return true
}
