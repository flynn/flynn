...
var githubRouter = FlynnDashboard.routers.github = new (Marbles.Router.createClass({
	...
	beforeHandler: function (event) {
		...
		if ( !FlynnDashboard.githubClient ) {
	...
	auth: function () {
		...
		if (view && view.constructor.displayName === "Views.GithubAuth" && view.isMounted()) {
			...
			view = FlynnDashboard.primaryView = React.renderComponent(
				FlynnDashboard.Views.GithubAuth(props),
				FlynnDashboard.el
			);
		}
	},
	...
	deploy: function (params) {
		...
		var view = FlynnDashboard.secondaryView = React.renderComponent(
			FlynnDashboard.Views.GithubDeploy(props),
...
githubRouter.dispatcherIndex = FlynnDashboard.Dispatcher.register(githubRouter.handleEvent.bind(githubRouter));
